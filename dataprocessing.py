...
def load_live_data():
	...
	im.save("Resized_face.jpg", quality = 95)
	im2 = imageio.imread("Resized_face.jpg")
	im3 = im2[:, :, 0]
	im3_flat = im3.flatten()

    for pixel in im3_flat.split( ):
    	temp_list.append(int(pixel))

    data = Zerocenter_ZCA_whitening_Global_Contrast_Normalize(temp_list)
    test_live_x.append(data.reshape(2304).tolist())
